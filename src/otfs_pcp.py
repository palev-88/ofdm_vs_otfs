...
@dataclass
class PCPOTFSConfig:
    """PCP-OTFS parameters: delay axis M (= active subcarriers), Doppler axis N, per-subsymbol CP length Mcp (= ZC pilot length), pilot column and guards, GCE-BEM order."""
    M: int = 256
    N: int = 14
    Mcp: int = 18
    scs_hz: float = 60e3
...
def zadoff_chu(length, root=1):
    """Zadoff-Chu sequence of the given length and root (cf = length mod 2)."""
    n = np.arange(length)
    cf = length % 2
...
class PCPOTFSTransceiver:
    """PCP-OTFS transceiver: inverse-Zak TX with per-subsymbol CP; RX = ZC correlation (Stage 1), GCE-BEM smoothing (Stage 2), CP-redundancy noise estimate, FDE with pilot cancellation, Zak detection (report Sec. 7)."""
    ...
    def tx(self, qam_symbols):
        """Map data + ZC pilot onto the DD grid, inverse-Zak along Doppler, add per-subsymbol CP; returns (frame samples, DD grid)."""
        cfg = self.cfg
    ...
    def rx(self, rx_signal, noise_var):  # noise_var unused — rx() self-estimates from CP redundancy; kept for call-site compatibility
        """Full receiver: noise from CP redundancy, two-stage channel estimation, FDE with pilot cancellation, Zak; returns (data symbols, H_f, None)."""
        cfg = self.cfg
    ...
    def rx_true_csi(self, rx_signal, fading, delay_samples, noise_var):
        """Genie-CSI reference: build H_f from the true fading taps at each subsymbol midpoint, then run the same FDE detector."""
        cfg = self.cfg
        M, N, MT = cfg.M, cfg.N, cfg.MT
        H_f = np.zeros((M, N), dtype=complex)
        for n in range(N):
    ...
    def count_data_res(self):
        """Number of data resource elements on the DD grid."""
        return self._n_data
