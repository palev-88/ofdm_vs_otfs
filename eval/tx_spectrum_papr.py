...
BW_CONFIGS = {
    'NB-A': dict(M=256,  SCS=30e3, Mcp_g=8,  ZP=14),
    'WB-B': dict(M=1024, SCS=60e3, Mcp_g=40, ZP=99),
}
...
def measure_aclr(f, Pxx, occ_bw, ch_bw):
    ...
    def band_pow(f_center, bw):
        ...
        """Integrate the PSD over [f_lo, f_hi] (Riemann sum times bin spacing)."""
...
def run_one_config(cfg_label, cfg, outdir):
    ...
    for method in methods:
        ...
        fs_up = Fs * OSR
        # Fine RBW for the ACLR integrals (edge smearing at coarse RBW would
        # corrupt the adjacent-channel power); coarse RBW (above the SCS) for
        # the display, so the PCP variants' deterministic pilot comb is
        # averaged rather than resolved -- the figure verifies the mask, not
        # sub-SCS fine structure.
        f_psd, Pxx = psd_welch(sig_up, fs_up)
        f_plot, P_plot = psd_welch(sig_up, fs_up,
                                   nperseg=min(1024, len(sig_up) // 8))
...
def plot_psd(results, cfg_label, outdir):
    ...
    # Annotate regions
    # Label placed deep in the empty passband region so it never collides
    # with the curves, which sit near 0 dB after mean normalisation.
    ax.text(0, -50, 'PASSBAND\n(all methods\nsuperimposed)',
            ha='center', va='center', fontsize=10, color='#1a6a1a',
    ...
    for m, r in results.items():
        fP, PP = r.get('f_plot', r['f_psd']), r.get('P_plot', r['Pxx'])
        Pxx_db = 10 * np.log10(PP + 1e-30)
        f_mhz = fP / 1e6
        # Normalise to the mean passband level, not the peak: a boosted pilot
        # line would otherwise shift the whole data floor down for that
        # method, faking a level offset between equal-power waveforms.
        inband = np.abs(fP) <= 0.4 * occ_bw
        ref = 10 * np.log10(np.mean(PP[inband]) + 1e-30)
        mask = np.abs(f_mhz) <= x_span
        ax.plot(f_mhz[mask], Pxx_db[mask] - ref, color=COLORS[m],
    ...
    ax.set_ylabel('Normalized PSD (dB, passband mean = 0)')
